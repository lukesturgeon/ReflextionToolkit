...
void testApp::setup(){
	...
	bounds = ofRectangle(200, 300.0f, ofGetWidth()-400, 100);
	dancer = ofRectangle(200, 400.0f, bounds.width, 100);
	spectator = ofRectangle(200, 500.0f, bounds.width, 100);
	
	// load the audio tracks
	dancerStory.loadSound("audio/dancer_story_1min.mp3");
	dancerStory.setVolume(HIGH_VOLUME);
	dancerStory.setPan(0.0f); // set to middle
	
	spectatorStory.loadSound("audio/spectator_story_1min.mp3");
...
//				dancerStory.setPosition(0.95f);
//				spectatorStory.setPosition(0.95f);
...
void testApp::draw(){
	...
	if (transitionState == TRANSITION_LOCK && currentFrameIndex == 4) {
		...
		if (currentPerspective == DANCER_PERSPECTIVE) {
			dancerOffset = fftVolume * 100.0f;
		}
		else if (currentPerspective == SPECTATOR_PERSPECTIVE) {
			spectatorOffset = fftVolume * 100.0f;
		}
		...
		futuraLight48.drawString(s, (int)(ofGetWidth()*0.5f - (rect.width*0.5f) ), 464);
...
//			ofRect(spectatorDataLine[i]);
		...
		// calculate the total length once (we don't need to keep re-calculating this)
		int totalTime = ofMap(memoryPlayheadX, bounds.x, bounds.x+bounds.width, 0, 60);//185 = TOTAL_AUDIO_LENGTH / 1000;
		int s = totalTime % 60;
		totalTime /= 60;
		int m = totalTime % 60;
	...
	ofSetLineWidth(3);
	ofLine(underlineStart, 90.5f, underlineEnd, 90.5f);
...
//--------------------------------------------------------------
void testApp::goToFrame(int nextFrame) {
	ofLogNotice() << "GOTO: " << nextFrame;
	
	if (transitionState == TRANSITION_READY) {
		transitionState = TRANSITION_OUT;
		nextFrameIndex = nextFrame;
	}
	else {
...
void testApp::resetExperience() {
	...
	transitionState = TRANSITION_READY;
	currentFrameIndex = 0;
	underlineStart = underlineTargets[0][0];
	underlineEnd = underlineTargets[0][1];
	dancerStory.stop();
	spectatorStory.stop();
	csv.clear();
